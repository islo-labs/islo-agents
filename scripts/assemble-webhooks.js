#!/usr/bin/env node
/**
 * Assemble webhooks/<source>-*.json from agents/<role>/rules/<source>.json fragments.
 *
...
function collectRules(source) {
  ...
  for (const agent of agents) {
    const path = join(agentsDir, agent, "rules", `${source}.json`);
    ...
    rules.push(...fragment);
    console.log(`+ ${agent}/rules/${source}.json (${fragment.length} rule(s))`);
  }
  return rules;
}
