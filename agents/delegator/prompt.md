...
|--------|-------------|------------------------|
| Implement / fix / address feedback | `linear-task` / `agents/task` | `implement-<ISSUE>` or `linear-task-<issue-id>` |
...
- Implement from a Linear-shaped request: `islo job run linear-task --param …` when you have issue fields; otherwise create an issue-scoped sandbox from `islo-stack` and start `agents/task` via the harness.
